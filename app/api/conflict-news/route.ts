...
const KEYWORDS: Record<string, string[]> = {
  ukraine: ["ukraine", "kyiv", "zelensky", "kremlin", "donbas", "kharkiv", "russia war", "russian invasion"],
  middleeast: ["gaza", "israel", "hamas", "hezbollah", "iran", "palestinian", "houthi", "netanyahu"],
...
export async function GET() {
  ...
  const tagged = allArticles
    .map((a) => ({ ...a, conflict: matchConflict(a.title + " " + a.snippet) }))
  ...
  return NextResponse.json(data, {
    headers: {
      // CDN caches for 15 min, serves stale for 30 min while revalidating
